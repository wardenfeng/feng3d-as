package me.feng3d.fagal.fragment
{
	...
	public function F_TerrainDiffusePostLighting():void
	{
		...
		mul(uvTemp, _.uv_v, _.tile_fc_vector.c(0)); // 缩放uv （表示将重复x*x遍） x=scaleRegister.c(0)

		tex(_.finalColor_ft_4, uvTemp, _.texture_fs); // 使用默认贴图 得到默认颜色值
		tex(blendTemp, _.uv_v, _.blendingtexture_fs); // 计算混合纹理

		for (var i:int = 0; i < numSplattingLayers; ++i)
		{
			mul(uvTemp, _.uv_v, _.tile_fc_vector.c(i + 1)); //  缩放uv （表示将重复x*x遍） x=scaleRegister.c(i + 1)
			tex(uvTemp, uvTemp, _.terrainTextures_fs_array.getReg(i)); // 使用地面纹理 得到该纹理颜色值

			sub(uvTemp, uvTemp, _.finalColor_ft_4); // uvtemp = uvtemp - targetreg; --------------1
			mul(uvTemp, uvTemp, blendTemp.c(i)); // uvtemp = uvtemp * blendtemp; ----------2  (0 <= blendtemp <= 1)
			add(_.finalColor_ft_4, _.finalColor_ft_4, uvTemp); // 添加到默认颜色值上  targetreg = targetreg + uvtemp; ------------3
