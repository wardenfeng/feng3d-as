package me.feng3d.animators
{
	...
	import me.feng3d.animators.states.SkeletonClipState;
	...
	public class SkeletonAnimator extends Animator
	{
		...
		private var _activeSkeletonState:SkeletonClipState;

		/** 全局变换矩阵缓冲类型编号 */
		protected var globalmatricesBufferTypeId:String = Context3DBufferTypeID.GLOBALMATRICES_VC_VECTOR;
		/** 全局变换矩阵缓冲 */
		protected var globalmatricesBuffer:VCVectorBuffer;
		...
		public function SkeletonAnimator(animationSet:SkeletonAnimationSet, skeleton:Skeleton, forceCPU:Boolean = false)
		{
			...
			if (forceCPU)
			{
				animationSet.cancelGPUCompatibility();
			}
		...
		{
			globalmatricesBuffer = new VCVectorBuffer(globalmatricesBufferTypeId, updateGlobalmatricesBuffer);
		}

		override public function collectCache(context3dCache:Context3DCache):void
		{
			context3dCache.addDataBuffer(globalmatricesBuffer);
		}

		override public function releaseCache(context3dCache:Context3DCache):void
		{
			context3dCache.removeDataBuffer(globalmatricesBuffer);
		}

		private function updateGlobalmatricesBuffer():void
		{
			globalmatricesBuffer.update(_globalMatrices, _numJoints * 3);
		}
		...
		public function play(animationName:String, offset:Number = NaN):void
		{
			if (_activeAnimationName != animationName)
			{
				...
				//获取活动的骨骼状态
				_activeNode = _animationSet.getAnimation(animationName);
				...
				_activeSkeletonState = _activeState as SkeletonClipState;
			}
